...
export async function getAssignmentDescriptionRecommendations({
  ...
  try {
    ...
    // Create structured prompt for Gemini
    const exactPrompt = `
    Generate 3 professional assignment descriptions for a ${level} level ${result?.name} course titled "${title}"
    
    Original Description (for reference):
    ${desc || "No description provided"}

    Requirements:
    1. Each assignment should:
       - Be 50-200 words long
       - Include clear objectives
       - Specify deliverables
       - Match the course level (${level})
       - Use professional language

    2. Consider level-specific requirements:
       - Beginner: Step-by-step instructions, basic concepts
       - Intermediate: Applied knowledge, moderate complexity
       - Advanced: Complex problems, industry-standard requirements

    Return a JSON array with exactly 3 objects in this format:
    [
      {
        "value": "Complete assignment description",
    ...
    try {
      ...
      console.error("Error parsing AI response:", parseError);
      return generateFallbackAssignments(title, level);
    }
  } catch (error) {
...
// Fallback assignment generator
function generateFallbackAssignments(title, level) {
  const templates = {
    Beginner: {
      desc: `Create a basic ${title} project following the provided guidelines. Focus on implementing fundamental concepts and best practices.`,
      reqs: [
        "Follow the project structure",
        "Implement basic features",
        "Add proper documentation",
      ],
      delivs: ["Source code", "README file", "Project demo"],
    },
    Intermediate: {
      desc: `Develop a ${title} solution that demonstrates your understanding of intermediate concepts. Include error handling and optimizations.`,
      reqs: [
    ...
    Advanced: {
      desc: `Build a production-ready ${title} implementation that showcases advanced concepts and industry best practices.`,
      reqs: [
        "Implement complex features",
        "Add comprehensive tests",
        "Deploy solution",
      ],
      delivs: ["Production code", "API documentation", "Deployment guide"],
    },
  };

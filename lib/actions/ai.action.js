...
// --- FIXED generateQuizzes function ---
export async function generateQuizzes({ desc, title, category, level }) {
  // --- Fetch category name outside the main try block ---
  const categoryName = category?.name || "General"; // Default category name

  try {
    const exactPrompt = `
    Generate 5 quiz questions for a ${level} level ${categoryName} course titled "${title}"

    Course Description:
    ${desc || "No description provided"}

    Requirements:
    1. Each quiz question must include:
       - A clear question related to the course topic and description.
       - 4 multiple-choice options labeled A, B, C, D.
       - The correct answer identified by its label (A, B, C, or D).
       - A brief explanation of why the correct answer is right.
       - Each question should be distinct and relevant to the course content.

    2. The questions should be appropriate for the specified level:
       - Beginner: Focus on fundamental concepts, definitions, basic usage.
       - Intermediate: Application of concepts, practical scenarios, comparisons.
       - Advanced: Complex problem-solving, nuanced understanding, edge cases, synthesis.

    3. Questions should cover diverse aspects mentioned in the course description if available.

    4. Avoid trivial or overly simple questions. Ensure options are plausible distractors.

    Return ONLY a valid JSON array (no markdown, no introductory text) with exactly 5 objects in this specific format:
    [
      {
        ...
        "explanation": "Brief explanation why C is the correct answer."
      }
      // ... (4 more objects like this)
    ]
    `;
    ...
    try {
      ...
      // Validate the structure of the parsed suggestions
      if (!Array.isArray(suggestions) || suggestions.length === 0) {
        throw new Error("AI response is not a non-empty array of questions.");
      }

      // Validate and transform suggestions
      const transformedQuizzes = suggestions
        .map((s) => {
          // Basic validation
          if (
            !s ||
            ...
            Object.keys(s.options).length !== 4 ||
            !s.correctAnswer ||
            !["A", "B", "C", "D"].includes(s.correctAnswer)
          ) {
            console.warn("Skipping invalid suggestion from AI:", s);
            return null;
          }

          // Transform options
          const transformedOptions = Object.entries(s.options).map(
            ([key, optionText]) => ({
              option: String(optionText || ""),
              isCorrect: key === s.correctAnswer,
            }),
          );

          // Ensure exactly one correct answer
          const correctCount = transformedOptions.filter(
            (opt) => opt.isCorrect,
          ).length;
          if (correctCount !== 1) {
            console.warn(
              "Skipping suggestion due to incorrect number of correct answers:",
              s,
            );
            return null;
          }
      ...
      // If no valid questions after filtering, fallback
      if (transformedQuizzes.length === 0) {
        console.warn(
          "AI generated no valid questions after filtering. Falling back.",
        );
        return generateFallbackQuizzes(title, level, categoryName);
      }

      // If fewer than 5 valid questions, supplement with fallback
      if (transformedQuizzes.length < 5) {
        console.warn(
          `AI generated only ${transformedQuizzes.length} valid questions. Supplementing with fallback.`,
        );
        ...
        const needed = 5 - transformedQuizzes.length;
        for (let i = 0; i < needed; i++) {
          // Simple append, could add logic to avoid duplicates if necessary
          transformedQuizzes.push(fallbackQuizzes[i % fallbackQuizzes.length]);
        }
      }

      return transformedQuizzes.slice(0, 5); // Ensure exactly 5
    } catch (parseError) {
      console.error(
        "Error parsing AI response:",
        parseError,
      ...
      console.warn("Falling back due to JSON parse error."); // Added log
      return generateFallbackQuizzes(title, level, categoryName);
    }
  } catch (error) {
    // <--- CATCH BLOCK FOR 503 (and other API errors)
    console.error("Error generating quiz suggestions from AI:", error); // Keep this log
    // --- FIXED: Call fallback instead of returning [] ---
    console.warn(
      "AI quiz generation failed (API error or other). Using fallback quizzes.",
    ); // Added log
    return generateFallbackQuizzes(title, level, categoryName); // Use the categoryName fetched earlier
    // --- END FIXED ---
  }
}

function generateFallbackQuizzes(title, level, category) {
  // Raw templates with { A: ..., B: ... } options and correctAnswer key
  const rawTemplates = {
    Beginner: [
      {
        question: `What is the primary purpose of the course "${title}"?`,
        options: {
          A: "To teach advanced data analysis techniques",
          B: "To provide a comprehensive guide to project management",
          C: `To introduce the fundamental concepts of ${category}`,
          D: "To focus on optimizing software performance",
        },
        correctAnswer: "C",
        explanation: `Beginner courses typically focus on introducing core concepts of the subject, like ${category}.`,
      },
      {
        question: `Which of these is a core concept usually covered in a beginner ${category} course like "${title}"?`,
        options: {
          A: "Advanced machine learning model deployment",
          B: "Quantum computing algorithms",
          C: `Basic principles and terminology of ${category}`,
          D: "Complex multi-threaded application design",
        },
        correctAnswer: "C",
        explanation: `Beginner courses establish foundational knowledge, including basic principles and terminology specific to ${category}.`,
      },
    ],
    Intermediate: [
      {
        question: `In the context of "${title}", how might one apply intermediate ${category} skills in a practical scenario?`,
        options: {
          A: `By developing a simple application or solving a common problem using ${category} techniques`,
          B: "By designing a large-scale distributed system from scratch",
          C: "By inventing a completely new algorithm",
          D: "By only reading theoretical papers",
        },
        correctAnswer: "A",
        explanation: `Intermediate level focuses on applying learned concepts to practical, moderately complex tasks within ${category}.`,
      },
      {
        question: `What is a key benefit of mastering the intermediate ${category} topics covered in "${title}"?`,
        options: {
          A: "Ability to solve any problem instantly",
          B: `Enhanced efficiency and capability in handling ${category}-related tasks`,
          C: "Complete elimination of the need for further learning",
          D: "Guaranteed job offer at a top tech company",
        },
        correctAnswer: "B",
        explanation: `Intermediate skills in ${category} typically lead to improved efficiency and effectiveness in relevant tasks.`,
      },
    ],
    Advanced: [
      {
        question: `How can the advanced ${category} techniques from "${title}" be used to optimize complex systems?`,
        options: {
          A: "By randomly changing code until it works",
          B: "By applying basic, generic solutions",
          C: `By leveraging deep ${category} knowledge for performance tuning and sophisticated problem-solving`,
          D: "By ignoring performance metrics",
        },
        correctAnswer: "C",
        explanation: `Advanced ${category} knowledge allows for nuanced optimization and tackling complex system challenges effectively.`,
      },
      {
        question: `What potential limitation should be considered when applying advanced ${category} concepts from "${title}"?`,
        options: {
          A: "Advanced concepts have no limitations",
          B: "They are always the simplest solution",
          C: "They might introduce complexity or have specific trade-offs (e.g., performance vs. maintainability) in certain contexts",
          D: "They are universally applicable without modification",
        },
        correctAnswer: "C",
        explanation: `Advanced ${category} concepts often come with trade-offs and require careful consideration of the specific context and potential limitations.`,
      },
    ],
  };

  // Select the appropriate raw template based on level
  const selectedRawTemplate = rawTemplates[level] || rawTemplates.Beginner;

  // Transform the selected raw template into the QuizForm format
  const transformedTemplate = selectedRawTemplate.map((q) => {
    const transformedOptions = Object.entries(q.options).map(
      ([key, optionText]) => ({
        option: optionText,
    ...
    return {
      question: q.question,
      options: transformedOptions,
      // Discard explanation and correctAnswer from the final object
    };
  });

  // Duplicate questions from the transformed template to get exactly 5 questions
  const generatedQuizzes = [];
  for (let i = 0; i < 5; i++) {
    // Use modulo operator to cycle through the available questions in the template
    const index = i % transformedTemplate.length;
    // Push a deep copy to avoid modifying the template objects if needed later
    generatedQuizzes.push(
      JSON.parse(JSON.stringify(transformedTemplate[index])),
    );
  }

  return generatedQuizzes; // Return the array of 5 questions in the correct format
}

...
export async function getAssignmentByCourse(slug) {
  try {
    ...
    const [assignment] = await Assignment.aggregate([
      // Step 1: Match the course by slug to get the course _id
      {
        $lookup: {
          ...
          pipeline: [
            ...
            },
            {
              $project: {
                _id: 1,
                title: 1, // Include title for population
              },
            },
          ],
          as: "courseData",
      ...
      // Step 2: Unwind courseData to filter assignments by matched course
      {
        $unwind: {
          path: "$courseData",
      ...
      // Step 3: Match assignments where course matches the found course _id
      {
        $match: {
          $expr: {
            $eq: ["$course", "$courseData._id"],
          },
        },
      },
      // Step 4: Lookup instructor data
      {
        $lookup: {
          from: "instructor", // Collection name for Instructor model
          localField: "instructor",
          foreignField: "_id",
          as: "instructorData",
        },
      },
      // Step 5: Unwind instructorData
      {
        $unwind: {
          path: "$instructorData",
          preserveNullAndEmptyArrays: true,
      ...
      // Step 6: Project the final output
      {
        $project: {
          ...
          // Include other assignment fields (adjust based on your schema)
          title: 1,
          description: 1,
          dueDate: 1,
          totalMarks: 1,
      ...
      // Step 7: Limit to one result (mimic findOne)
      {
        $limit: 1,
      },
    ]);
